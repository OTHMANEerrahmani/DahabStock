...
pub async fn get_catalogue_complet() -> Result<String, String> {
    ...
    let data = CatalogueService::get_catalogue_complet(&mut client).await.map_err(|e| crate::utils::format_sql_error(&e.to_string()))?;
...
pub async fn add_article_standard(payload_str: String) -> Result<String, String> {
    let payload: ArticleStandardPayload = serde_json::from_str(&payload_str).map_err(|e| crate::utils::format_sql_error(&e.to_string()))?;
    let mut client = get_connection().await.map_err(|e| crate::utils::format_sql_error(&e.to_string()))?;
...
pub async fn add_barre_aluminium(payload_str: String) -> Result<String, String> {
    let payload: BarreAluminiumPayload = serde_json::from_str(&payload_str).map_err(|e| crate::utils::format_sql_error(&e.to_string()))?;
    let mut client = get_connection().await.map_err(|e| crate::utils::format_sql_error(&e.to_string()))?;
