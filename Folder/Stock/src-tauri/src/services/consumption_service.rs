...
impl ConsumptionService {
    pub async fn consume_barre(client: &mut Client<Compat<TcpStream>>, projet_id: i32, materiau_id: i32, quantite_length: f64, preneur: &str, operation_id: Option<&str>) -> Result<()> {
        ...
        if let Some(c) = chute {
            ...
            let cons_id = ConsommationRepository::create_consommation(
                client, projet_id, materiau_id, 0.0, quantite_length, Some(c.chute_id), preneur, "Chute", operation_id
            ).await?;
            ...
            let cons_id = ConsommationRepository::create_consommation(
                client, projet_id, materiau_id, 1.0, quantite_length, None, preneur, "Nouvelle Barre", operation_id
            ).await?;
    ...
    pub async fn consume_standard(client: &mut Client<Compat<TcpStream>>, projet_id: i32, materiau_id: i32, quantite: f64, preneur: &str, operation_id: Option<&str>) -> Result<()> {
        StockService::update_stock_principal(client, materiau_id, -quantite as i32).await?;
        
        let cons_id = ConsommationRepository::create_consommation(
            client, projet_id, materiau_id, quantite, 0.0, None, preneur, "Standard", operation_id
        ).await?;
