...
export default function EntreeStock() {
  ...
  const [lignes, setLignes] = useState<LigneReception[]>([{ id: Date.now(), materiau_ref: '', quantite_recue: 1, prix_achat: 0 }]);
  ...
  const addLigne = () => {
    setLignes([...lignes, { id: Date.now(), materiau_ref: '', quantite_recue: 1, prix_achat: 0 }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const l of lignes) {
      ...
      const mat = materiaux.find(m => m.reference.toLowerCase() === l.materiau_ref.toLowerCase());
      if (!mat) {
    ...
    try {
      ...
      if (parsed.status === 'success') {
        ...
        setNumeroBR('');
        setFournisseurNom('');
        setLignes([{ id: Date.now(), materiau_ref: '', quantite_recue: 1, prix_achat: 0 }]);
        setTimeout(() => {
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{marginBottom: '2rem'}}>
                ...
                <div style={{display: 'flex', flexDirection: 'column', gap: '1rem'}}>
                  {lignes.map((ligne) => {
                    const isCodeUnknown = ligne.materiau_ref !== '' && !materiaux.find(m => m.reference.toLowerCase() === ligne.materiau_ref.toLowerCase());
                    return (
