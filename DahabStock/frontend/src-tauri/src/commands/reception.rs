...
pub async fn get_fournisseurs() -> Result<String, String> {
    ...
    let data = ReceptionRepository::get_fournisseurs(&mut client).await.map_err(|e| e.to_string())?;
...
pub async fn add_fournisseur(nom: String) -> Result<String, String> {
    let mut client = get_connection().await.map_err(|e| e.to_string())?;
    let new_id = ReceptionRepository::create_fournisseur(&mut client, &nom).await.map_err(|e| e.to_string())?;
...
pub async fn get_materiaux() -> Result<String, String> {
    let mut client = get_connection().await.map_err(|e| e.to_string())?;
    let data = ReceptionRepository::get_materiaux(&mut client).await.map_err(|e| e.to_string())?;
    let res = ApiResponse { status: "success".to_string(), data: Some(data), error: None };
    Ok(serde_json::to_string(&res).unwrap())
}
...
pub async fn get_historique_receptions() -> Result<String, String> {
    let mut client = get_connection().await.map_err(|e| e.to_string())?;
    let data = ReceptionRepository::get_historique_receptions(&mut client).await.map_err(|e| e.to_string())?;
    let res = ApiResponse { status: "success".to_string(), data: Some(data), error: None };
    Ok(serde_json::to_string(&res).unwrap())
}
...
pub async fn submit_reception(payload_str: String) -> Result<String, String> {
    let payload: BonReceptionPayload = serde_json::from_str(&payload_str).map_err(|e| e.to_string())?;
