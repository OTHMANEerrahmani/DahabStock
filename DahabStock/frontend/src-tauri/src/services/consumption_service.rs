...
impl ConsumptionService {
    pub async fn consume_barre(client: &mut Client<Compat<TcpStream>>, projet_id: i32, materiau_id: i32, quantite_length: f64) -> Result<()> {
        ...
        if let Some(c) = chute {
            ...
            let cons_id = ConsommationRepository::create_consommation(
                client, projet_id, materiau_id, "StockChutes", None, Some(quantite_length), Some(c.chute_id)
            ).await?;
            ...
            let cons_id = ConsommationRepository::create_consommation(
                client, projet_id, materiau_id, "StockPrincipal", Some(1.0), Some(quantite_length), None
            ).await?;
